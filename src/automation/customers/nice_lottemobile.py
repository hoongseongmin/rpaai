...
def process_nice_lottemobile(operation: str, customer: str, diff_file_path: Path, output_dir: Path, options: dict) -> dict:
    """나이스CMS 롯데백화점모바일상품권 전용 생성 프로세스"""
    options = options or {}
    
    # === 1번 작업: 청구공문 (Word) ===
    if operation in ['nice_lottemobile_1', 'doc_official']:
        templates_dir = output_dir.parent / "templates"
        
        context = {
            '작업명': 'nicecms_lottemobile',
            '공문번호': '',
            '합계금액': '0',
            '공급가액': '0',
            '부가세': '0'
        ...
        excel_path = templates_dir / "나이스CMS" / "롯데백화점모바일상품권" / "[설정]_공문.xlsx"
        ...
        template_path = templates_dir / "나이스CMS" / "롯데백화점모바일상품권" / "[양식]_공문.docx"
        if not template_path.exists(): 
            return {"success": False, "message": f"워드 양식 파일을 찾을 수 없습니다.\n경로: {template_path}"}
            
        month_str = report_date.replace(' ', '')
        file_prefix = f"나이스CMS_롯데모바일_청구공문_{month_str}"
        ...
        try:
            ...
            return {"success": True, "output": str(output_path), "summary": {"message": "작업 완료: 롯데백화점모바일상품권 청구공문이 생성되었습니다."}}
