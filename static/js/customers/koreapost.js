...
window.RPA_CUSTOMERS['koreapost'] = {
    monthOffset: -1, // 전월 기준
    tasks: [
        { id: 'koreapost_all', op: 'kp_task_all', title: '🌟 [통합] 1~4번 엑셀 한 파일로 묶어서 생성', date: '매월 10일' },
        { id: 'koreapost_1', op: 'kp_task1', title: '1. 📊 이전설치 공문 (Excel)', date: '매월 5일' },
        { id: 'koreapost_2', op: 'kp_task2', title: '2. 📊 이행실적 공문 (Excel)', date: '매월 5일' },
    ...
    renderOneclick: function(savedData) {
        const reloValue = savedData['이전설치내역'] || '';
        const isNone = reloValue === '없음';
        return `
        <!-- 🟧 1번 문서 전용 -->
        <div style="background: #fff8f0; border: 1px solid #fde6ce; border-radius: 8px; padding: 20px; width: 100%; box-sizing: border-box; margin-top:15px;">
            <h4 style="color: #e67e22; margin-top: 0; margin-bottom: 15px; font-size: 1.1em;">🟧 [1번 문서 전용] 이전설치 공문 정보</h4>
            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 15px;">
                <div style="flex: 1; min-width: 200px;">
                    <label style="font-weight: bold; display: block; margin-bottom: 5px; color: #155724;">합계금액</label>
                    <input type="text" class="common-input" id="fast_total" data-key="합계금액" data-name="합계금액" value="${savedData['합계금액'] || ''}" oninput="formatMoney(this); calculateVAT(this); const t=document.querySelector('input[data-key=\\'당초청구금액_3\\']'); if(t){t.value=this.value; calculateClaimAmount('3');}" style="width: 100%; padding: 10px; border: 1px solid #fbdba7; border-radius: 4px; font-weight: bold; background-color: #fff;">
                </div>
                <div style="flex: 1; min-width: 200px;">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 15px;">
                <div style="flex: 1; min-width: 200px;">
                    <label style="font-weight: bold; display: flex; align-items: center; margin-bottom: 5px; color: #155724;">
                        이전설치내역
                        <label style="cursor: pointer; margin-left: 10px; display: flex; align-items: center; font-weight: normal; font-size: 0.9em; color: #155724;">
                            <input type="checkbox" onchange="const inp=document.getElementById('fast_relocation_detail'); if(this.checked){inp.value='없음';inp.readOnly=true;inp.style.backgroundColor='#fdf5e6';inp.style.color='#888';}else{inp.value='';inp.readOnly=false;inp.style.backgroundColor='#fff';inp.style.color='';}" ${isNone ? 'checked' : ''}>
                            <span style="margin-left: 5px;">이전설치 없음</span>
                        </label>
                    </label>
                    <input type="text" class="common-input" id="fast_relocation_detail" data-key="이전설치내역" data-name="이전설치내역" value="${reloValue}" placeholder="이전설치 내역을 입력하세요 (없으면 위 체크박스 선택)" style="width: 100%; padding: 10px; border: 1px solid #fbdba7; border-radius: 4px; font-weight: bold; background-color: ${isNone ? '#fdf5e6' : '#fff'}; color: ${isNone ? '#888' : ''};" ${isNone ? 'readonly' : ''}>
                </div>
            </div>
        </div>
        ...
        <div style="background: #f4fbf5; border: 1px solid #ccecd4; border-radius: 8px; padding: 20px; width: 100%; box-sizing: border-box; margin-top:15px;">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 15px;">
                ...
                <div style="flex: 1; min-width: 200px;">
                    ...
                    <input type="text" class="common-input" data-key="미사용차감금액" data-name="미사용차감금액" value="${savedData['미사용차감금액'] !== undefined ? savedData['미사용차감금액'] : '0'}" oninput="formatMoney(this); calculateActualPayment(); const t=document.querySelector('input[data-key=\\'정산감액_4\\']'); if(t){t.value=this.value; calculateClaimAmount('4');}" style="width: 100%; padding: 10px; border: 1px solid #ffeeba; border-radius: 4px; font-weight: bold; background-color: #fff;">
                </div>
                <div style="flex: 1; min-width: 200px;">
    ...
    onOneclickRendered: function() {
        ...
        if (typeof syncValue === 'function') {
            syncValue('합계금액', '당초청구금액_3');
            syncValue('합계금액', '청구금액_3');
            syncValue('미사용차감금액', '정산감액_4');
        }
    ...
    getDashboardKeys: function(operation, customerId) {
        if (customerId === 'koreapost_1') return ['합계금액', '공급가액', '부가세', '이전설치내역'];
    ...
    renderDashboardInput: function(key, value, operation, customerId) {
        ...
        if (key === '이전설치내역' && customerId === 'koreapost_1') {
            const isNone = value === '없음';
            return `
            <div style="margin-bottom: 5px;">
                <label style="cursor: pointer; display: inline-flex; align-items: center; font-size: 0.9em; font-weight: normal; color: var(--text);">
                    <input type="checkbox" onchange="const inp=document.getElementById('dash_relo_detail_2'); if(this.checked){inp.value='없음';inp.readOnly=true;inp.style.backgroundColor='#f5f5f5';inp.style.color='#888';}else{inp.value='';inp.readOnly=false;inp.style.backgroundColor='#f4faff';inp.style.color='';}" ${isNone ? 'checked' : ''}>
                    <span style="margin-left: 5px;">이전설치 없음</span>
                </label>
            </div>
            <input type="text" class="edit-input" id="dash_relo_detail_2" data-key="${key}" value="${value}" placeholder="이전설치내역을 입력하세요" style="width: 100%; padding: 10px; border: 2px solid var(--primary); border-radius: 4px; box-sizing: border-box; font-weight: bold; background-color: ${isNone ? '#f5f5f5' : '#f4faff'}; color: ${isNone ? '#888' : ''};" ${isNone ? 'readonly' : ''}>`;
        }
        return null;
    },
    ...
    fetchInitialData: function(fetchFn) {
        fetchFn('koreapost_2', 'kp_task2', '월유지비용', '월유지비용');
    },

    getActualTaskId: function(customer, operation) {
        if (customer === 'koreapost' && operation.startsWith('kp_task') && operation !== 'kp_task5') {
            const taskNum = operation.replace('kp_task', '');
            return `koreapost_${taskNum}`;
        }
        return customer;
    }
};